...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    if(isset($_POST["btnpay"])!="")
    {
         ...
         $up ="update tbl_payment set payment_status='".$_GET['pt']."', payment_date=curdate() where site_id='".$_GET["sid"]."'";
        if($conn->query($up))
        {
            $upD="update tbl_site set site_status='".$_GET['st']."' where site_id=".$_GET['sid'];
            if($conn->query($upD))
            {
            ?>
            <script>
